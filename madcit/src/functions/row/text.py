...
calc.registered = True



def regexpcountdistance(pattern,expression,start = 0,min = 0.5,multiply = 1):
    """
    ...
    count = 0
    if start == 0:
        for i in re.finditer(pattern,expression,re.UNICODE):
            count += 1.0/(len(expression[:i.start()].split())+1)

  
        return count
    else:
        for i in re.finditer(pattern,expression,re.UNICODE):
            count += 1.0/(len(expression[i.end():].split())+1)

        return count

regexpcountdistance.registered=True



def csvparse(*args):
    spamreader = csv.reader(args, delimiter=',', quotechar='"')
    li = tuple('c'+str(x) for x in xrange(16))
    yield li
    for row in spamreader:
        yield row


csvparse.registered=True
...
def textreferences(txt,maxlen = 5,pattern = r'(?:\b|_)(?:(?:1[5-9]\d{2,2})|(?:20\d{2,2}))(?:\b|_)|(?:\bet al\b)|(?:\bhttps*\:\b)|(?:^\[\d+\])' ):
    """
...
def firstnlines(txt):
    ...
    if exp.count('\n')<10:
        return exp
    references = []
    reversedtext = iter(exp.split('\n')[:20])
    return '\n'.join(reversedtext)

firstnlines.registered =True
